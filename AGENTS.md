...
| `/mypage` | `pages/MyPage/MyPage`     | 로그아웃 / 회원탈퇴 (비밀번호 재확인 모달) — 인증 가드 인라인 |
...
- ✅ 마이페이지 (`/mypage`) — 로그아웃 / 회원탈퇴(비밀번호 재확인 모달). 로그인/회원가입 성공 시 자동 이동
- ✅ 회원탈퇴 API (`DELETE /api/members/me`) 연동
- ✅ Mock API 모드 (`VITE_USE_MOCK_API=true`) — 백엔드 없이 인메모리 핸들러로 전체 시퀀스 검증. 시드 `test@dallyrun.com` / `Test1234!@`

**미구현 (의도적 스코프 밖)**

- ❌ `ProtectedRoute` 컴포넌트화 — `/mypage` 는 페이지 내부 인라인 가드(useEffect)로 처리 중. 보호 페이지가 늘어나면 추출 예정.
...
- `VITE_USE_MOCK_API=true` 로 설정하면 `apiRequest()` 가 백엔드 fetch 대신 `src/api/mockApi.ts` 의 인메모리 핸들러로 분기. 페이지·스토어·`auth.ts`·`authedRequest.ts` 는 변경 없이 동일하게 동작 — 백엔드 없이 회원가입 → 로그인 → 마이페이지 → 로그아웃 → 회원탈퇴 전체 시퀀스를 UI 에서 끝까지 검증할 수 있다.
