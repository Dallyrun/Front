...
열려있는 PR 또는 현재 브랜치의 변경점을 `.agents/agents/pr-reviewer.md` 지침에 맞춰 리뷰한다.

## 동작

1. `.agents/agents/pr-reviewer.md` 를 우선 읽고 리뷰어 역할로 전환한다.
2. 컨텍스트 수집:
...
4. 결과를 Markdown 체크리스트로 출력. 위반/의심 항목은 **정확한 파일:라인** 을 포함.
5. 리뷰어는 직접 수정/커밋/푸시하지 않는다.

## 참고

- 리뷰어 지침: `.agents/agents/pr-reviewer.md`
- 프로젝트 지침: `AGENTS.md`
