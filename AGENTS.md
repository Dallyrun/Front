...
- 어떠한 작업이든 시작 전 반드시 **Plan Mode** 로 전환해 계획을 세우고, 사용자 승인 후 진행한다.
  - 단순히 텍스트로 계획을 쓰는 것이 아니라, Codex 의 실제 **Plan Mode** 기능을 사용한다.
  - 작업 시작 시 `EnterPlanMode` 툴을 호출해 계획을 세우고, `ExitPlanMode` 로 사용자 승인을 받은 뒤 구현한다.
  - 사용자가 계획을 승인하기 전에는 파일 수정, 명령 실행, 브랜치 생성, PR 생성 등 실제 작업을 시작하지 않는다.
...
│  └─ webSettings.ts   # 웹 mock 표시 설정(localStorage), 언어/거리 단위 정규화와 포맷 helper
├─ asset/
...
- ✅ 웹 mock 표시 설정 — 백엔드 없이 `dallyrun-web-settings` localStorage 로 한국어/English, km/mile 단위 설정 저장 및 주요 웹 화면 표시 반영
...
| `webSettings.ts`  | `useWebSettings`, `normalizeWebSettings`, `formatDistanceText`, `translate`         | 웹 mock 표시 설정 저장/정규화, km↔mile 표시 변환, 한국어/영어 UI copy 매핑   |
...
- PR 리뷰 코멘트를 확인해 실제 코드/문서에 반영했다면, 해당 GitHub 리뷰 코멘트에 `Done` 이라고
  답글을 남긴다.
- 반영하지 않기로 판단한 리뷰 코멘트에는 `Done` 을 남기지 않고, 최종 응답에 이유만 짧게 설명한다.
